...
print ('   ****************************************************')
print ('   *         NanoVNA data file viewer  v1.0         *      (c) YeS 2018')
print ('   ****************************************************')
...
for line in handle:  # Loop by all lines in file
    if line.startswith("#"):  # Searching comments
        ...
        float_list = [np.float(words_in_line[1]), np.float(words_in_line[2]), np.float(words_in_line[3]), np.float(words_in_line[4])]
...
labels = ['Re S11','Im S11','Re S21','Im S21']


for i in range(set_num):
    plt.figure()
    plt.plot(frequency[:], data[i][:], linewidth='1.50', label= labels[i]) #  , label=filename[k]
    plt.xlabel('Frequency, MHz')
    plt.ylabel('Parameter')
    plt.suptitle('Measured parameters', fontsize=12, fontweight='bold')
    ...
    pylab.savefig(newpath + '/' + str(i) + '_parameter.png', bbox_inches='tight', dpi=200)
    # plt.show()
    plt.close('all')
